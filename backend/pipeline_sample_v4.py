"""
pipeline_sample_v4.py
named_reactions*.zip 전체 배치 파이프라인 v4

v3 대비 변경사항:
1. RECITATION 재시도 로직 (최대 2회 재시도)
2. reaction_family_name Title Case 정규화
3. 전체 132장 모드 지원 (--all 플래그)
4. 샘플 15장 기본값 유지 (하위 호환)
5. SMILES 정책 유지 (smiles_confidence < 0.85 → NULL)
6. staging only, reaction_cards 절대 변경 없음
"""
...
For toc/meta_explanatory: return empty scheme_candidates=[].
...
def to_title_case(s: str) -> str:
    ...
    for i, w in enumerate(words):
        # 하이픈 포함 단어 처리 (Simmons-Smith, 2-aza-Cope 등)
        if '-' in w:
            parts = w.split('-')
            result.append('-'.join(p.capitalize() for p in parts))
        else:
            w_lower = w.lower()
            if i == 0 or w_lower not in _LOWER_WORDS:
...
def canonical_family(raw: str) -> str:
    """reaction_family_name을 Title Case로 정규화"""
    if not raw:
        return raw
...
def normalize_response(raw) -> dict:
    ...
    result.update(raw)
    if result["page_kind"] not in {"toc","meta_explanatory","reaction_content","unknown"}:
    ...
    for sc in result["scheme_candidates"]:
        ...
        for ext in sc["extracts"]:
            if not isinstance(ext, dict):
                continue
            ext = dict(ext)
            # family name 정규화 (페이지 family로 통일)
            raw_ef = ext.get("reaction_family_name")
            if page_family:
                ext["reaction_family_name"] = page_family
            elif raw_ef:
                ext["reaction_family_name"] = canonical_family(raw_ef)
            # SMILES 보수화
...
# ── Gemini 호출 (재시도 포함) ───────────────────────────────────
def _call_once(img_data, api_key, user_prompt, model=MODEL):
...
def gemini_call_with_retry(img_data, api_key, user_prompt):
    """
    RECITATION 재시도 전략:
    1차: 원래 프롬프트
    2차: 동일 프롬프트 재시도 (RETRY_SLEEP 대기)
    3차: 보수 fallback 프롬프트
    """
    last_err = None
    for attempt in range(MAX_RETRIES + 1):
        try:
            if attempt == 0:
                raw = _call_once(img_data, api_key, user_prompt)
            elif attempt == 1:
                time.sleep(RETRY_SLEEP)
                raw = _call_once(img_data, api_key, user_prompt)
            else:
                time.sleep(RETRY_SLEEP)
                raw = _call_once(img_data, api_key, FALLBACK_PROMPT)
            return normalize_response(raw)
        except RecitationError as e:
            last_err = e
            print(f"(RECITATION retry {attempt+1}/{MAX_RETRIES+1})", end=" ", flush=True)
            continue
        except Exception as e:
            raise e
    # 모든 재시도 실패
    raise Exception(f"RECITATION after {MAX_RETRIES+1} attempts")

# ── delete-and-rebuild ─────────────────────────────────────────
def delete_batch(conn, batch):
...
def main():
    ...
    parser.add_argument("--db",      default="labint_round9_patched_v3.db")
    parser.add_argument("--batch",   default="sample_batch1")
    parser.add_argument("--n",       type=int, default=DEFAULT_SAMPLE,
                        help="Number of pages (default 15 for sample)")
    parser.add_argument("--all",     action="store_true",
                        help="Process all pages in ZIP (132장 전체)")
    ...
    mode = "전체" if args.all else f"샘플 {args.n}장"
    print(f"=== pipeline_sample_v4 ({PIPELINE_VER}) ===")
    print(f"ZIP: {zip_path}  |  DB: {args.db}")
    print(f"Prompt: {PROMPT_VERSION}  |  Model: {MODEL}")
    print(f"Batch: {args.batch}  |  Mode: {mode}")
    ...
    if args.rebuild:
        ...
        print(f"[rebuild] Deleted {n} pages")
    ...
    if args.all:
        target_imgs = all_imgs
    else:
        abbrev  = [f for f in all_imgs if page_no(f) < 46]
        reaction = [f for f in all_imgs if page_no(f) >= 46]
        target_imgs = abbrev[:3] + reaction[:args.n - 3]

    print(f"\n처리 대상 {len(target_imgs)}장 (p{page_no(target_imgs[0])}~p{page_no(target_imgs[-1])})\n")

    MF_FIELDS = ["page_no","image_filename","page_kind","status",
                 "scheme_count","extract_count","reaction_family_name","error"]
    ...
    with zipfile.ZipFile(zip_path) as z:
        for fname in target_imgs:
            ...
            pi_id = conn.execute(
                ...
                (source_zip, fname)).fetchone()["id"]

            # 2. Gemini 호출 (재시도 포함)
            try:
                result = gemini_call_with_retry(img_data, api_key, user_prompt)
            except Exception as e:
                err = str(e)[:400]
                print(f"FAIL: {err}")
                conn.execute(
            ...
            print(f"OK | kind={page_kind} | "
                  f"family={str(family or 'null')[:30]} | schemes={n_schemes}")

            # 3. toc/meta_explanatory: DB row 0개 (placeholder 절대 금지)
            ...
            # 4. reaction_content: scheme별 1행 INSERT
            ...
            for sch in schemes:
                ...
                for ext in extracts:
                    rfname = ext.get("reaction_family_name") or family
                    ts3 = now_iso()
                    conn.execute("""
                        INSERT INTO reaction_extracts
                        (scheme_candidate_id,
                         reaction_family_name, reaction_family_name_norm,
                         extract_kind, transformation_text,
            ...
            manifest_rows.append({
                "page_no": pno, "image_filename": fname,
                "page_kind": page_kind, "status": "ok",
                "scheme_count": n_schemes, "extract_count": total_extracts,
    ...
    for r in conn.execute(
        ...
        "WHERE ingest_batch=? GROUP BY page_kind", (args.batch,)
    ).fetchall():
    ...
    rows = conn.execute("""
        ...
        GROUP BY pi.page_no, sc.scheme_index, re.reaction_family_name
    ...
    # raw JSON dump
    sfx = "all" if args.all else args.batch
    dump_path = Path(args.db).parent / f"raw_json_dump_{sfx}_v4.json"
    ...
    # manifest
    mf_path = Path(args.db).parent / f"sample_manifest_{sfx}_v4.csv"
