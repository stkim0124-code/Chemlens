...
ChemLens named-reaction family coverage automation — v5.
...
------
# dry-run
python gemini_family_automation_v5.py --plan-only --db app\\labint.db \\
    --stage-db app\\labint_v4_stage.db --report-dir reports\\v4

# 목표치까지 full run
python gemini_family_automation_v5.py \\
    --db app\\labint.db \\
...
_PSEUDO_FAMILY_KEYWORDS = (
    ...
    "general concepts", "general concept", "appendix", "overview of", "introduction to", "background", "glossary", "index",
)
...
def select_run_mode(candidates: list[Candidate], forced_mode: str = "auto") -> str:
    if forced_mode == 'force-frontier':
        return 'frontier_discovery'
    if forced_mode == 'force-bulk':
        return 'bulk_discovery'
    if not candidates:
        return 'bulk_discovery'
    if is_discovery_frontier(candidates):
        return 'frontier_discovery'
    top = candidates[: min(12, len(candidates))]
    seed_ready = sum(1 for c in top if c.seed_ready)
    deterministic = sum(1 for c in top if c.lane == 'deterministic')
    if seed_ready >= 3 or deterministic >= 4:
        return 'seed_assisted_bulk'
    return 'bulk_discovery'
...
def insert_family_seed(conn: sqlite3.Connection, cand: Candidate,
    ...
    """, (
        ...
        reuse_meta.get("extractor_prompt_version") if reuse_meta else "v5",
        extraction_raw_json,
...
def main() -> int:
    ...
    p.add_argument("--stage-db", default="app/labint_v5_stage.db",
    ...
    p.add_argument("--report-dir", default="reports/v5",
    ...
    p.add_argument("--frontier-max-candidate-limit", type=int, default=6,
                   help="Adaptive upper bound for frontier discovery candidates per round")
    p.add_argument("--frontier-grow-after", type=int, default=2,
                   help="Increase frontier candidate limit after this many full-success rounds")
    p.add_argument("--zero-progress-stop-rounds", type=int, default=1,
                   help="Stop after this many consecutive rounds with inserted=0 in frontier mode")
    p.add_argument("--bulk-zero-progress-stop-rounds", type=int, default=2,
                   help="Stop after this many consecutive rounds with inserted=0 in bulk mode")
    p.add_argument("--diagnostic-every-rounds", type=int, default=3,
                   help="Run diagnostic benchmark every N successful rounds (0=disabled)")
    p.add_argument("--mode", choices=["auto", "force-frontier", "force-bulk"], default="auto",
                   help="Run mode selection policy")
    args = p.parse_args()
    ...
    frontier_candidate_limit = args.discovery_candidate_limit
    frontier_success_streak = 0
    ...
    try:
        while rounds_run < args.max_rounds:
            ...
            mode = select_run_mode(candidates, args.mode)
            if mode == 'frontier_discovery':
                active_candidates = candidates[:frontier_candidate_limit]
            else:
            ...
            if not candidates:
                consecutive_empty += 1
                print(f"[round {rounds_run}] no candidates (consecutive_empty={consecutive_empty})")
            ...
            if args.plan_only:
                ...
                for c in candidates[:20]:
                    print(f"  lane={c.lane:14s} prio={c.priority_score:3d} manual={c.manual_rows} covered={c.existing_queryable_count} extracts={c.extract_rows} seed={1 if c.seed_ready else 0} page={c.page_kind} {c.family_name}")
                if mode == 'frontier_discovery':
                    print(f"[plan-only] discovery frontier detected: limiting real run to first {len(active_candidates)} candidate(s) per round")
                elif mode == 'seed_assisted_bulk':
                    print("[plan-only] seed-assisted bulk mode detected")
                else:
                    print("[plan-only] bulk discovery mode detected")
            ...
            if mode == 'frontier_discovery':
                ...
                print(f"[mode] seed-assisted bulk: trying {len(active_candidates)} candidate(s) with batch_size={args.batch_size}")
            else:
                print(f"[mode] bulk discovery: trying {len(active_candidates)} candidate(s) with batch_size={args.batch_size}")
            ...
            round_inserted = process_round(
                ...
                total_inserted,
                discovery_mode=(mode == 'frontier_discovery'),
            )
            ...
            if round_inserted == 0:
                ...
                frontier_success_streak = 0
                if mode == 'frontier_discovery' and consecutive_zero_progress >= args.zero_progress_stop_rounds:
                    stopped_reason = "zero_insert_round_discovery"
                    break
                if mode != 'frontier_discovery' and consecutive_zero_progress >= args.bulk_zero_progress_stop_rounds:
                    stopped_reason = "zero_progress_rounds"
                    break
            else:
                consecutive_zero_progress = 0
                if mode == 'frontier_discovery' and round_inserted >= len(active_candidates):
                    frontier_success_streak += 1
                    if frontier_success_streak >= args.frontier_grow_after and frontier_candidate_limit < args.frontier_max_candidate_limit:
                        frontier_candidate_limit += 1
                        frontier_success_streak = 0
                        print(f"[mode] frontier limit increased to {frontier_candidate_limit}")
                else:
                    frontier_success_streak = 0

            if (not args.plan_only and diagnostic_benchmark_file and diagnostic_benchmark_file.exists()
                    and args.diagnostic_every_rounds > 0 and round_inserted > 0
                    and rounds_run % args.diagnostic_every_rounds == 0):
                diag = run_benchmark(stage_db, diagnostic_benchmark_file, report_dir, f"diagnostic_round{rounds_run:02d}")
                if 'error' not in diag:
                    noisy_cases = diag.get('noisy_cases')
                    meaningful_cases = diag.get('meaningful_cases')
                    print(f"[diagnostic] round={rounds_run} top1={diag.get('top1_accuracy')} top3={diag.get('top3_accuracy')} noisy={diag.get('noisy_pass')}/{noisy_cases} meaningful={diag.get('meaningful_pass')}/{meaningful_cases}")
        ...
        if diagnostic_benchmark_file and diagnostic_benchmark_file.exists() and diagnostic_benchmark_file != benchmark_file:
            final_diagnostic = run_benchmark(stage_db, diagnostic_benchmark_file, report_dir, "diagnostic_final")
        summary['final_diagnostic_benchmark'] = final_diagnostic
...
def attempt_gemini(conn, cand: Candidate, gemini: GeminiClient, args) -> InsertResult | None:
    ...
    out_family = str(payload.get("reaction_family_name", "")).strip()
    if out_family and _norm(out_family) != _norm(cand.family_name):
        return InsertResult(success=False, error=f"gemini: family mismatch ({out_family})")
