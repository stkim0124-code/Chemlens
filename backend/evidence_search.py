...
def _family_coarse_profile(family_name: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    # ── Gemini salvage 8-family coarse gate (2026-04-18 patch) ─────────
    # Buchner cluster: 이 5개 family는 diazo+arene / ring_expansion 쿼리에서
    # 강하게 penalty를 받아야 한다. Buchner benchmark case 납치를 방지한다.
    _BUCHNER_CLUSTER_FORBID = {
    ...
    if fam in _BUCHNER_CLUSTER_FORBID:
        return {
            'need_any': [],
            'boost_any': [],
    ...
    # Barton cluster: 이 3개 family는 decarboxylation / deoxygenation 쿼리에서
    # 강하게 penalty를 받아야 한다. Barton benchmark case 납치를 방지한다.
    _BARTON_CLUSTER_FORBID = {
        'enyne metathesis',
        'hofmann-loffler-freytag reaction',
        'mitsunobu reaction',
    }
    if fam in _BARTON_CLUSTER_FORBID:
        return {
            'need_any': [],
            'boost_any': [],
