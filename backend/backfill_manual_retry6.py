...
def clear_page_results(conn, page_image_id: int):
    sc_ids = [r[0] for r in conn.execute("SELECT id FROM scheme_candidates WHERE page_image_id=?", (page_image_id,)).fetchall()]
    if sc_ids:
        ph = ",".join("?" * len(sc_ids))
...
def main():
    ...
    ap.add_argument("--raw-json", default="raw_json_dump_manual_retry6_v5.json")
    ap.add_argument("--only-pages-csv", default="retry6_manual_pages.csv")
    ap.add_argument("--zip-dir", default=None)
    ap.add_argument("--zip", default=None)
    ap.add_argument("--tag", default="manual_retry6")
    args = ap.parse_args()
        ...
        for tgt in raw.get("targets", []):
            ...
            if row is None:
                rows.append({
                    ...
                    "error": f"page_images row not found for batch={args.batch}"
                })
                fail += 1
                continue
            ...
            try:
                ...
                mod.insert_schemes(conn, row["id"], normalized, source_zip, page_no)
                conn.execute(
                ...
                out_targets.append({"source_zip": source_zip, "page_no": page_no, "image_filename": image_filename, "result": normalized})
                print(f"[{source_zip} | p{page_no:3d}] manual backfill OK | schemes {schemes_before}->{schemes_after} | extracts {extracts_before}->{extracts_after}")
                ok += 1
                ...
                rows.append({
                    "source_zip": source_zip, "page_no": page_no, "image_filename": image_filename,
                    "status": "fail", "schemes_before": 0, "extracts_before": 0,
                    "schemes_after": 0, "extracts_after": 0, "error": str(e)[:500]
                })
                print(f"[{source_zip} | p{page_no:3d}] manual backfill FAIL | {str(e)[:120]}")
                fail += 1
    finally:
        conn.close()

    raw_out = {"run_at": now_iso(), "batch": args.batch, "tag": args.tag, "targets": out_targets}
    ...
    print("=" * 60)
    print(f"manual backfill done: ok={ok}, fail={fail}")
