...
TAGS = ['phase5_shallow_top5_completion_']
...
def alias_residue(conn: sqlite3.Connection, family: str) -> Dict[str, List[str]]:
    if family == 'Alder (Ene) Reaction (Hydro-Allyl Addition)':
        residues = ['Alder Ene Reaction', 'Alder (ene) Reaction']
    elif family == 'Amadori Reaction / Rearrangement':
        residues = ['Amadori Rearrangement']
    elif family == 'Arbuzov Reaction (Michaelis-Arbuzov Reaction)':
        residues = ['Arbuzov Reaction']
    elif family == 'Aza-Claisen Rearrangement (3-Aza-Cope Rearrangement)':
        residues = ['Aza-Claisen Rearrangement']
    else:
        residues = []
...
def verify_family(conn: sqlite3.Connection, family: str) -> Dict[str, Any]:
    ...
    curated = conn.execute(
        ...
        FROM reaction_extracts
        WHERE reaction_family_name=? AND (
            notes_text LIKE ?
        )
        ORDER BY id
        ''',
        (family, '%phase5_shallow_top5_completion_%'),
    ).fetchall()
    return {
        'family': family,
        **dict(row),
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Verify shallow-family sprint round 1.')
    ap.add_argument('--db', default='app/labint.db')
    ap.add_argument('--report-dir', default='reports/family_completion_phase5_shallow_top5_verify')
    args = ap.parse_args()
    ...
    try:
        ...
        jpath = report_dir / 'family_completion_phase5_shallow_top5_verify.json'
        mpath = report_dir / 'family_completion_phase5_shallow_top5_verify.md'
        jpath.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding='utf-8')
        lines = ['# Family completion phase5 shallow-top5 verify','',f'- db: `{db_path}`','']
