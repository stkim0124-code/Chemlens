...
ALIAS_RESIDUES = {
    'Criegee Oxidation': ['Criegee Reaction'],
    'Danheiser Benzannulation': ['Danheiser Annulation'],
    'Danheiser Cyclopentene Annulation': [],
    'Darzens Glycidic Ester Condensation': ['Darzens Reaction'],
    "Davis' Oxaziridine Oxidations": ['Davis Oxaziridine Oxidation', 'Davis Oxaziridine Oxidations'],
}

TAG = 'phase10_shallow_top5_completion_'
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Verify shallow-family sprint phase10.')
    ap.add_argument('--db', default='app/labint.db')
    ap.add_argument('--report-dir', default='reports/family_completion_phase10_shallow_top5_verify')
    args = ap.parse_args()
    ...
    try:
        ...
        jpath = report_dir / 'family_completion_phase10_shallow_top5_verify.json'
        mpath = report_dir / 'family_completion_phase10_shallow_top5_verify.md'
        jpath.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding='utf-8')
        lines = ['# Family completion phase10 shallow-top5 verify', '', f'- db: `{db_path}`', '']
