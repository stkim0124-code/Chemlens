...
TARGETS: dict[str, dict[str, Any]] = {
    "Claisen Condensation / Claisen Reaction": {
        "cluster": "buchner",
        "guidance": [
            "Use a simple intermolecular Claisen condensation between two aliphatic esters to form a beta-keto ester. Avoid allyl vinyl ether Claisen rearrangement entirely.",
            "Use a mixed Claisen condensation with one simple aliphatic ester electrophile and one simple ester enolate donor. Keep everything acyclic and aliphatic.",
            "Use the simplest textbook beta-keto ester forming Claisen condensation you can. No arene, no diazo, no phosphorus, no rearrangement, no cyclic substrate.",
        ],
        "forbid": "diazo_arene_combo, ring_expansion, Buchner-like arene-carbene insertion, allyl-vinyl-ether Claisen rearrangement",
    },
    "Krapcho Dealkoxycarbonylation": {
        "cluster": "buchner",
        "guidance": [
            "Use a textbook Krapcho dealkoxycarbonylation on a simple beta-keto ester or malonate derivative, giving a one-ester-lost product. Keep everything acyclic and aliphatic.",
            "Use a minimal Krapcho example with a geminal diester or beta-keto ester that loses one alkoxycarbonyl group under salt/wet polar conditions. Avoid arene and diazo motifs.",
            "Use the simplest RDKit-parseable Krapcho example you can. No diazo, no arene-carbene insertion, no ring expansion, no pericyclic rearrangement.",
        ],
        "forbid": "diazo_arene_combo, ring_expansion, Buchner-like arene-carbene insertion, pericyclic rearrangement",
    },
}
...
def _build_prompt(family: str, stage_re: sqlite3.Row, stage_mols: list[sqlite3.Row], attempt_idx: int) -> str:
    cfg = TARGETS[family]
    ...
    guidance = cfg["guidance"][min(attempt_idx - 1, len(cfg["guidance"]) - 1)]
    return (
        ...
        f"Hard constraints:\n"
        f"- {guidance}\n"
        f"- Avoid these collision signals: {cfg['forbid']}.\n"
...
def _is_already_applied(conn: sqlite3.Connection, family: str) -> bool:
    row = conn.execute(
        "SELECT 1 FROM extract_molecules WHERE queryable=1 AND reaction_family_name=? AND structure_source='gemini_rebuild_seed' LIMIT 1",
        (family,),
    ).fetchone()
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Continuous rebuild loop for a single remaining rejected family using frozen apply reuse.")
    ap.add_argument("--family", default="Claisen Condensation / Claisen Reaction")
    ...
    ap.add_argument("--report-dir", default=str(SCRIPT_DIR / "reports" / "gemini_rebuild_single_family_loop"))
    ...
    if family not in TARGETS:
        print(f"[ERROR] unsupported family: {family}")
        print(f"Supported: {', '.join(TARGETS)}")
        return 1
    ...
    report_dir = report_root / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{_safe_name(family)[:32]}"
    ...
    print("=" * 72)
    print("CONTINUOUS GEMINI REBUILD LOOP FOR SINGLE FAMILY")
    print("=" * 72)
    ...
    if _is_already_applied(conn, family):
        state = _state(conn)
        conn.close()
        print(f"[already_applied] {family}")
        summary = {
            "family": family,
            "already_applied": True,
            "final_state": state,
            "final_benchmark": baseline,
        }
        (report_dir / "single_family_loop_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"summary: {report_dir / 'single_family_loop_summary.json'}")
        return 0
    state_start = _state(conn)
    conn.close()
    ...
    for round_idx in range(1, args.max_rounds + 1):
        ...
        print("\n" + "-" * 72)
        print(f"[ROUND {round_idx}] {family}")
        print("-" * 72)

        round_rec: dict[str, Any] = {"round": round_idx, "attempts": []}
        round_applied = False

        for attempt_idx in range(1, args.max_attempts + 1):
            prompt = _build_prompt(family, stage_re, stage_mols, attempt_idx)
            (round_dir / f"attempt_{attempt_idx:02d}_prompt.txt").write_text(prompt, encoding="utf-8")
            try:
                raw_text, api_resp = _request_gemini(api_key, args.model, prompt)
                (round_dir / f"attempt_{attempt_idx:02d}_api_response.json").write_text(json.dumps(api_resp, ensure_ascii=False, indent=2), encoding="utf-8")
                (round_dir / f"attempt_{attempt_idx:02d}_raw.txt").write_text(raw_text, encoding="utf-8")
                ...
                msg = str(e)
                (round_dir / f"attempt_{attempt_idx:02d}_error.txt").write_text(msg, encoding="utf-8")
                print(f"  attempt {attempt_idx}: ERROR {msg[:180]}")
                round_rec["attempts"].append({"attempt": attempt_idx, "status": "error", "error": msg})
                continue

            tmpdir = Path(tempfile.mkdtemp(prefix="rebuild_one_family_"))
            tmpdb = tmpdir / "labint.db"
            shutil.copy2(canonical, tmpdb)
            tmpconn = _connect(tmpdb)
            try:
                new_extract_id, new_ids = _insert_candidate_pair(tmpconn, stage_re, stage_mols, cand, "gemini_rebuild_seed")
                tmpconn.commit()
                bench = _run_benchmark(backend_root, tmpdb, benchmark, round_dir, f"attempt_{attempt_idx:02d}_bench")
            except Exception as e:
                msg = str(e)
                (round_dir / f"attempt_{attempt_idx:02d}_insert_or_bench_error.txt").write_text(msg, encoding="utf-8")
                print(f"  attempt {attempt_idx}: INSERT/BENCH ERROR {msg[:180]}")
                round_rec["attempts"].append({"attempt": attempt_idx, "status": "insert_or_bench_error", "error": msg, "candidate": cand})
                try:
                    tmpconn.close()
                except Exception:
                    pass
            ...
            ok = _baseline_ok(bench)
            print(f"  attempt {attempt_idx}: top1={bench['top1_accuracy']:.4f} top3={bench['top3_accuracy']:.4f} violations={bench['disallow_top3_violations']} {'PASS' if ok else 'FAIL'}")
            attempt_rec = {
                "attempt": attempt_idx,
                "status": "pass" if ok else "fail",
                "candidate": cand,
                "benchmark": bench,
                "stage_extract_id": int(stage_re["id"]),
                "new_extract_id": int(new_extract_id),
                "new_molecule_ids": new_ids,
            }
            round_rec["attempts"].append(attempt_rec)

            if ok:
                (round_dir / f"attempt_{attempt_idx:02d}_frozen_candidate.json").write_text(json.dumps(attempt_rec, ensure_ascii=False, indent=2), encoding="utf-8")
                backup = canonical.with_name(f"labint.backup_before_{_safe_name(family)[:24]}_round{round_idx:02d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
                shutil.copy2(canonical, backup)
                print(f"[backup] {backup.name}")
                real_conn = _connect(canonical)
                try:
                    if _is_already_applied(real_conn, family):
                        print(f"[{family}] already present as gemini_rebuild_seed -> SKIP")
                    else:
                        _insert_candidate_pair(real_conn, stage_re, stage_mols, cand, "gemini_rebuild_seed")
                        real_conn.commit()
                        final_bench = _run_benchmark(backend_root, canonical, benchmark, round_dir, f"round_{round_idx:02d}_post_apply_bench")
                        if not _baseline_ok(final_bench):
                            raise RuntimeError(
                                f"post-apply benchmark regressed: top1={final_bench.get('top1_accuracy')} top3={final_bench.get('top3_accuracy')} violations={final_bench.get('disallow_top3_violations')}"
                            )
                    ...
                    print(f"[{family}] APPLY ERROR -> restored backup: {e}")
                    round_rec["apply_error"] = str(e)
                finally:
                    real_conn.close()
                shutil.rmtree(tmpdir, ignore_errors=True)
                break
        ...
        round_rec["applied"] = round_applied
        rounds.append(round_rec)
        if round_applied:
            empty_rounds = 0
            break
        empty_rounds += 1
        print(f"[ROUND {round_idx} RESULT] applied=0 empty_rounds={empty_rounds}")
    ...
    final_applied = _is_already_applied(final_conn, family)
    final_conn.close()
    ...
    summary_path = report_dir / "single_family_loop_summary.json"
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    print("\n" + "=" * 72)
    print("[FINAL RESULT]")
    print(f"  applied: {final_applied}")
