...
MANUAL_ALIAS_OVERRIDES = {
    ...
    # --- Phase 3a-bis v2 additions (20260421_061442) ---
    'Baeyer-Villiger Oxidation/rearrangement': 'Baeyer-Villiger Oxidation/Rearrangement',  # id=13 -> id=291 (case variant on /rearrangement tail)
    'Barton Radical Decarboxylation': 'Barton Radical Decarboxylation Reaction',  # id=44 -> id=43 (missing Reaction suffix)
    'Simmons-Smith Reaction': 'Simmons-Smith Cyclopropanation',  # id=151 -> id=199 (generic -> mechanism-specific)
    'Suzuki Cross-Coupling': 'Suzuki Cross-Coupling (Suzuki-Miyaura Cross-Coupling)',  # id=148 -> id=217 (full DB canonical with trailing Cross-Coupling)
    ...
    try:
        ...
        return conn.execute(
            ...
            SELECT id, reaction_family_name, extract_kind, reactant_smiles, product_smiles,
                   notes_text, transformation_text, reactants_text, products_text
