...
def _family_delta_adjustment(family_name: Optional[str], reaction_delta: Optional[Dict[str, Any]]) -> Tuple[float, List[str]]:
    ...
    # for Larock Indole, aldehyde + beta-ketoester + amine for Hantzsch).
        ...
        if p.get("pyrrole", 0) < 1:
            penalize(0.22, "Paal-Knorr Pyrrole: pyrrole 생성물 시그니처 부족")
        ...
        if p.get("pyrrole", 0) >= 1 and r.get("amine", 0) >= 1:
            boost(1.10, "Paal-Knorr Pyrrole: pyrrole + amine 조합 적합")
        ...
        # We accept EITHER product signature; missing both is strong anti-signal.
        has_dhp_product = (p.get("pyridine", 0) >= 1)
        if not has_dhp_product:
            penalize(0.28, "Hantzsch: pyridine/DHP 생성물 시그니처 부족")
        if r.get("carbonyl", 0) < 2:
        ...
        # Distinguishing signal: aryl_halide + alkyne + aromatic_amine together.
        if r.get("aryl_halide", 0) < 1:
            penalize(0.22, "Larock Indole: aryl halide 전구체 부족")
        if r.get("alkyne", 0) < 1:
            penalize(0.30, "Larock Indole: 알킨 파트너 부족")
        if r.get("aromatic_amine", 0) < 1:
            penalize(0.22, "Larock Indole: 방향족 아민(o-iodoaniline) 전구체 부족")
        if p.get("pyrrole", 0) >= 1 and r.get("aryl_halide", 0) >= 1 and r.get("alkyne", 0) >= 1:
            boost(1.10, "Larock Indole: indole 생성물 + aryl halide + alkyne 조합 적합")
