...
def _family_delta_adjustment(family_name: Optional[str], reaction_delta: Optional[Dict[str, Any]]) -> Tuple[float, List[str]]:
    ...
    # for Larock Indole, aldehyde + beta-ketoester + amine for Hantzsch).
        ...
        # 1,4-dicarbonyl + primary amine -> N-substituted pyrrole.
        # 3g-2a: softened pyrrole-product requirement (0.22 -> 0.50) because
        # curated Paal-Knorr DB rows sometimes have intermediate/open-chain
        # products. Still require the precursor signals (dicarbonyl + amine).
        ...
        if r.get("amine", 0) < 1:
            penalize(0.30, "Paal-Knorr Pyrrole: 1차 아민 전구체 부족")
        if p.get("pyrrole", 0) >= 1 and r.get("amine", 0) >= 1:
            boost(1.12, "Paal-Knorr Pyrrole: pyrrole + amine 조합 적합")
        ...
        # 3g-2a: dropped hard aromatic_amine requirement (curated DB rows often
        # have N-substituted anilines or amine parsed into separate molecule that
        # our feature SMARTS misses). Kept aryl_halide + alkyne as the two-factor
        # distinguishing signal; aromatic_amine is a BOOST, not a penalty.
        if r.get("aryl_halide", 0) < 1:
            penalize(0.22, "Larock Indole: aryl halide 전구체 부족")
        if r.get("alkyne", 0) < 1:
            penalize(0.30, "Larock Indole: 알킨 파트너 부족")
        if p.get("pyrrole", 0) >= 1 and r.get("aryl_halide", 0) >= 1 and r.get("alkyne", 0) >= 1:
            boost(1.12, "Larock Indole: indole 생성물 + aryl halide + alkyne 조합 적합")
        if r.get("aromatic_amine", 0) >= 1 and r.get("aryl_halide", 0) >= 1:
            boost(1.08, "Larock Indole: o-aryl halide aniline 전구체 시그니처")
