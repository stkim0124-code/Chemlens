...
_REACTION_FEATURE_SMARTS: Dict[str, str] = {
    ...
    # --- Phase 3c-b feature SMARTS start (20260421_094500) ---
    # Added to support Aza-Wittig / HWE / Julia-Lythgoe pattern guards.
    "azide": "[$([#7]=[#7+]=[#7-]),$([#7-][#7+]#[#7])]",
    "sulfone": "[SX4](=[OX1])(=[OX1])",
    "phosphonate": "[PX4](=[OX1])([OX2][#6])[OX2][#6]",
    # --- end Phase 3c-b feature SMARTS ---
    # --- Phase 3e-b-1 feature SMARTS start (20260421_110300) ---
    # Broad P/Si atom presence for Wittig-class / Sakurai / Peterson guards,
    # and hydroxamate signature for Lossen rearrangement discrimination.
    "phosphorus": "[#15]",
    "silicon": "[#14]",
    "hydroxamate": "[CX3](=[OX1])[NX3][OX2H1]",
    # --- end Phase 3e-b-1 feature SMARTS ---
    # --- Phase 3g-1 feature SMARTS start (20260421_150000) ---
    # New reagent-class atoms / functional groups needed by allylation-cluster
    # guards (Stille/Keck tin, Reformatsky zinc) and by a *real* alkyl-halide
    # signature (distinct from aryl_halide) to finally fix the Finkelstein
    # guard that was softened in 3e-b-3.
    "tin": "[#50]",
    "zinc": "[#30]",
    "alkyl_halide": "[CX4;!$(C=O);!$(C=N);!$(C=S)][F,Cl,Br,I]",
    # --- end Phase 3g-1 feature SMARTS ---
    # --- Phase 3g-2 feature SMARTS start (20260421_180000) ---
    # Anhydride signature for Perkin discriminator; heterocycle signatures for
    # Paal-Knorr Pyrrole/Furan, Feist-Benary Furan, Hantzsch Dihydropyridine,
    # and Larock Indole; aromatic_amine for Ullmann/Buchwald discriminators.
    "anhydride": "[CX3](=O)O[CX3](=O)",
    "pyrrole": "[nX3;r5]",
    "furan": "[oX2;r5]",
    "pyridine": "[nX2;r6]",
    "dihydropyridine": "[#7;r6;!a]",
    "aromatic_amine": "[c][NX3;H2,H1]",
    # Ammonia feature (3g-2c): our "amine" SMARTS [NX3;H2,H1] excludes NH3
    # because NH3 has H3. Hantzsch/PKP multicomponent reactions use bare "N"
    # (ammonia) as nitrogen source, so expose ammonia as a separate feature.
    "ammonia": "[NX3H3]",
    # --- end Phase 3g-2 feature SMARTS ---
    # --- Phase 3g-3 feature SMARTS start (20260421_220000) ---
    # Isocyanide R-NC: SMILES convention uses [C-]#[N+]. Distinguishes Ugi/
    # Passerini MCRs from Aldol/Mannich. Rejects nitriles (C#N, CC#N) which
    # lack the formal charges.
    "isocyanide": "[C-]#[N+]",
    # --- end Phase 3g-3 feature SMARTS ---
}
...
def _family_delta_adjustment(family_name: Optional[str], reaction_delta: Optional[Dict[str, Any]]) -> Tuple[float, List[str]]:
    ...
    # --- Phase 3c-b pattern guards start (20260421_094500) ---
    # Aza-Wittig / HWE / Julia-Lythgoe guards. These families are named for specific
    # reagent classes (azide, phosphonate, sulfone) that the DB SMARTS lookup now
    # exposes via the 3c-b SMARTS additions. Every branch is a defensive penalty
    # that fires only when the required reagent class is missing from the reactants.
    elif "aza-wittig" in fam:
        # Aza-Wittig uses an azide-derived iminophosphorane condensing with a C=O
        # to deliver an imine/amide; the azide signature must be present somewhere
        # on the reactant side.
        if r.get("azide", 0) < 1:
            penalize(0.25, "Aza-Wittig: 반응물에 azide 신호 없음")
        if r.get("carbonyl", 0) < 1:
            penalize(0.45, "Aza-Wittig: 카보닐 파트너 부족")
        if r.get("azide", 0) >= 1 and r.get("carbonyl", 0) >= 1:
            boost(1.15, "azide + 카보닐 조합 적합 — Aza-Wittig 적합")
    elif "horner-wadsworth-emmons" in fam or "horner wadsworth emmons" in fam or fam.startswith("hwe "):
        # HWE: stabilized phosphonate carbanion + aldehyde/ketone -> alkene + phosphate.
        if r.get("phosphonate", 0) < 1:
            penalize(0.22, "HWE: phosphonate 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.45, "HWE: 카보닐 파트너 부족")
        if d.get("alkene", 0) <= 0:
            penalize(0.45, "HWE: alkene 생성 신호 부족")
        if r.get("phosphonate", 0) >= 1 and r.get("carbonyl", 0) >= 1 and d.get("alkene", 0) > 0:
            boost(1.20, "phosphonate + carbonyl -> alkene 조합 적합 — HWE 적합")
    elif "julia" in fam and ("olefination" in fam or "lythgoe" in fam or "kocienski" in fam):
        # Julia / Julia-Lythgoe / Julia-Kocienski: sulfone-stabilized carbanion + aldehyde
        # -> alkene (sulfone is consumed).
        if r.get("sulfone", 0) < 1:
            penalize(0.25, "Julia: sulfone 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.45, "Julia: 카보닐 파트너 부족")
        if d.get("alkene", 0) <= 0:
            penalize(0.50, "Julia: alkene 생성 신호 부족")
        if r.get("sulfone", 0) >= 1 and r.get("carbonyl", 0) >= 1 and d.get("alkene", 0) > 0 and d.get("sulfone", 0) < 0:
            boost(1.20, "sulfone + carbonyl -> alkene 조합 적합 — Julia 적합")
    # --- end Phase 3c-b pattern guards ---
    # NOTE: Phase 3c-c (Wittig guard) attempted 2026-04-21 but rolled back —
    # see reports/phase3c/apply_3c_c_20260421_095500/apply_summary.json. The guard
    # displaced 7 Wittig bleeds but 7 new bleeds appeared as Wolff-Kishner / Perkin /
    # Henry picked up the slack, netting 0 confused_pairs reduction and a 1-case
    # top3 regression. Wittig greediness is downstream of broader bleed cluster;
    # fixing it surgically would require simultaneous guards on the replacement
    # families. Deferred pending Phase 3d depth sprint.
    # --- Phase 3e-b-1 pattern guards start (20260421_110300) ---
    # Batch 1: low-risk surgical guards targeting admission confused_pairs
    # identified by Phase 3e-a triage. Each elif inspects the predicted family's
    # required reagent-class signature and applies a defensive penalty (×≤1.0)
    # when the signature is absent from reactants+products.
    elif "wolff-kishner" in fam:
        # Wolff-Kishner reduces C=O to CH2 via hydrazone; requires a ketone/aldehyde.
        if r.get("carbonyl", 0) < 1:
            penalize(0.28, "Wolff-Kishner: 카보닐 전구체 부족")
        if d.get("carbonyl", 0) >= 0:
            penalize(0.35, "Wolff-Kishner: 카보닐 감소 신호 부족")
        else:
            boost(1.10, "Wolff-Kishner: 카보닐 감소 방향 적합")
    elif "hofmann rearrangement" in fam or ("hofmann" in fam and "rearrangement" in fam):
        # Hofmann rearrangement: primary amide -> amine (with loss of CO).
        if r.get("amide", 0) < 1:
            penalize(0.25, "Hofmann rearrangement: amide 전구체 부족")
        if d.get("amine", 0) <= 0 and d.get("amide", 0) >= 0:
            penalize(0.40, "Hofmann rearrangement: amide→amine 방향 불일치")
    elif "cope elimination" in fam or "cope reaction" in fam:
        # Cope elimination: amine N-oxide thermolysis -> alkene + hydroxylamine.
        if r.get("amine", 0) < 1:
            penalize(0.25, "Cope elimination: amine 전구체 부족")
        if d.get("alkene", 0) <= 0:
            penalize(0.40, "Cope elimination: alkene 생성 신호 부족")
    elif "henry" in fam and "reaction" in fam:
        # Henry (nitroaldol): nitroalkane + carbonyl -> β-nitro alcohol.
        if r.get("nitro", 0) < 1:
            penalize(0.22, "Henry: nitro 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.40, "Henry: 카보닐 파트너 부족")
        if r.get("nitro", 0) >= 1 and r.get("carbonyl", 0) >= 1 and d.get("alcohol", 0) > 0:
            boost(1.15, "Henry: nitro + carbonyl -> alcohol 조합 적합")
    elif "arndt-eistert" in fam:
        # Arndt-Eistert: carboxylic acid -> acyl chloride -> diazoketone -> Wolff
        # rearrangement -> homologated acid. Requires carboxylic_acid or acid_chloride.
        if (r.get("carboxylic_acid", 0) + r.get("acid_chloride", 0)) < 1:
            penalize(0.25, "Arndt-Eistert: 산/산염화물 전구체 부족")
    elif "lossen" in fam:
        # Lossen: hydroxamic acid -> isocyanate -> amine. Requires hydroxamate
        # or at least a carboxylic_acid/acid_chloride precursor.
        if r.get("hydroxamate", 0) < 1 and (r.get("carboxylic_acid", 0) + r.get("acid_chloride", 0)) < 1:
            penalize(0.25, "Lossen: hydroxamate/산 전구체 부족")
    elif "claisen condensation" in fam or "claisen reaction" in fam:
        # Claisen Condensation: ester + enolate/ester -> β-ketoester. Requires ester.
        if r.get("ester", 0) < 1:
            penalize(0.28, "Claisen Condensation: ester 전구체 부족")
        if d.get("carbonyl", 0) < 0 and d.get("ester", 0) < 0:
            penalize(0.35, "Claisen Condensation: ester/카보닐 감소 — 축합 방향 불일치")
    elif "acetoacetic ester synthesis" in fam:
        # Acetoacetic Ester Synthesis: ethyl acetoacetate + R-X (alkyl halide).
        if r.get("ester", 0) < 1:
            penalize(0.25, "Acetoacetic Ester Synthesis: ester 전구체 부족")
    elif "sakurai" in fam:
        # Sakurai allylation: allylsilane + carbonyl -> homoallyl alcohol.
        if r.get("silicon", 0) < 1:
            penalize(0.25, "Sakurai: silicon(allylsilane) 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.40, "Sakurai: 카보닐 파트너 부족")
    elif "paternò-büchi" in fam or "paterno-buchi" in fam or "paterno-büchi" in fam:
        # Paternò-Büchi [2+2]: carbonyl + alkene under hν -> oxetane.
        if r.get("carbonyl", 0) < 1:
            penalize(0.25, "Paternò-Büchi: 카보닐 전구체 부족")
        if r.get("alkene", 0) < 1 and d.get("alkene", 0) >= 0:
            penalize(0.35, "Paternò-Büchi: alkene 전구체 부족")
    elif "noyori asymmetric hydrogenation" in fam or ("noyori" in fam and "hydrogenation" in fam):
        # Noyori asymmetric hydrogenation: reduces C=C or C=O via BINAP-Ru/Rh.
        # Requires alkene or carbonyl substrate with reduction direction.
        if r.get("alkene", 0) < 1 and r.get("carbonyl", 0) < 1:
            penalize(0.25, "Noyori hydrogenation: alkene/carbonyl 기질 부족")
        if d.get("alkene", 0) >= 0 and d.get("carbonyl", 0) >= 0:
            penalize(0.35, "Noyori hydrogenation: 환원 방향 불일치")
    elif "ley oxidation" in fam or "ley-griffith" in fam or "tpap" in fam:
        # Ley (TPAP/NMO): alcohol -> carbonyl oxidation.
        if r.get("alcohol", 0) < 1:
            penalize(0.22, "Ley oxidation: 알코올 전구체 부족")
        if d.get("alcohol", 0) >= 0:
            penalize(0.28, "Ley oxidation: 알코올 감소 신호 부족")
        if d.get("carbonyl", 0) <= 0:
            penalize(0.30, "Ley oxidation: 카보닐 생성 신호 부족")
    elif "tsuji-wilkinson" in fam or ("tsuji" in fam and "decarbonylation" in fam):
        # Tsuji-Wilkinson decarbonylation: aldehyde -> R-H + CO via Rh catalyst.
        if r.get("carbonyl", 0) < 1:
            penalize(0.25, "Tsuji-Wilkinson: aldehyde 전구체 부족")
        if d.get("carbonyl", 0) >= 0:
            penalize(0.35, "Tsuji-Wilkinson: 카보닐 감소 방향 불일치")
    elif "pinacol" in fam and ("rearrangement" in fam or "semipinacol" in fam):
        # Pinacol / semipinacol rearrangement: vicinal diol (or β-hydroxy leaving
        # group) -> ketone via 1,2-migration. Requires alcohol substrate.
        if r.get("alcohol", 0) < 1:
            penalize(0.25, "Pinacol/Semipinacol: 알코올 전구체 부족")
        if d.get("carbonyl", 0) <= 0:
            penalize(0.35, "Pinacol/Semipinacol: 카보닐 생성 신호 부족")
    # --- end Phase 3e-b-1 pattern guards ---
    # --- Phase 3e-b-2 pattern guards start (20260421_130000) ---
    # Batch 2: Wittig-cluster focused. After 3e-b-1, 10 of 23 remaining
    # smarts_guard cases still predict "Wittig Reaction" because the
    # ylide/phosphorus signature is absent from reactants. Add a targeted
    # Wittig guard. To limit collateral damage (3c-c rollback observed
    # bleed to WK/Perkin/Henry — WK/Henry now guarded in 3e-b-1), add
    # defensive guards for Perkin, Schmidt, Swern/Corey-Kim, Friedel-Crafts
    # alkylation so displaced Wittig scores can't simply redistribute onto
    # unguarded neighbors.
    elif fam == "wittig reaction" or fam.startswith("wittig reaction -") or fam.startswith("wittig reaction "):
        # Wittig: phosphorus ylide (R3P=CR2) + carbonyl -> alkene + R3P=O.
        # Required feature: phosphorus present on reactant side or in products
        # (R3P=O byproduct). Most Wittig-bleed confusions have no P atoms at all.
        if r.get("phosphorus", 0) < 1 and p.get("phosphorus", 0) < 1:
            penalize(0.20, "Wittig: phosphorus 부재 — ylide 전구체/부산물 없음")
        if r.get("carbonyl", 0) < 1:
            penalize(0.40, "Wittig: 카보닐 파트너 부족")
        if d.get("alkene", 0) <= 0:
            penalize(0.45, "Wittig: alkene 생성 신호 부족")
        if r.get("phosphorus", 0) >= 1 and r.get("carbonyl", 0) >= 1 and d.get("alkene", 0) > 0:
            boost(1.15, "Wittig: phosphorus + carbonyl -> alkene 조합 적합")
    elif "perkin" in fam and "reaction" in fam:
        # Perkin Reaction: aromatic aldehyde + acid anhydride -> cinnamic acid.
        # Requires aromatic_ring + carbonyl + anhydride + carboxylic_acid product.
        # 3g-2: added anhydride requirement (was the single strongest missing
        # discriminator — HWE/Reformatsky/Stobbe/Strecker/Tishchenko lacked it
        # and were bleeding into Perkin in 3g-1b).
        if r.get("aromatic_ring", 0) < 1:
            penalize(0.28, "Perkin: 방향족 고리 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.35, "Perkin: 카보닐(aldehyde) 부족")
        if r.get("anhydride", 0) < 1:
            penalize(0.22, "Perkin: 산 무수물(anhydride) 전구체 부족")
        if p.get("carboxylic_acid", 0) < 1 and p.get("ester", 0) < 1:
            penalize(0.40, "Perkin: 계피산/에스터 생성 신호 부족")
    elif "schmidt" in fam and "reaction" in fam:
        # Schmidt: carboxylic acid (or ketone) + HN3 -> amine/amide with CO/N2 loss.
        # Require acid/acid_chloride or ketone substrate.
        if (r.get("carboxylic_acid", 0) + r.get("acid_chloride", 0) + r.get("carbonyl", 0)) < 1:
            penalize(0.30, "Schmidt: 산/산염화물/카보닐 기질 부족")
    elif "swern" in fam:
        # Swern Oxidation: alcohol -> carbonyl via DMSO/(COCl)2/Et3N.
        if r.get("alcohol", 0) < 1:
            penalize(0.25, "Swern: 알코올 전구체 부족")
        if d.get("alcohol", 0) >= 0:
            penalize(0.30, "Swern: 알코올 감소 신호 부족")
        if d.get("carbonyl", 0) <= 0:
            penalize(0.30, "Swern: 카보닐 생성 신호 부족")
    elif "corey-kim" in fam or ("corey" in fam and "kim" in fam):
        # Corey-Kim Oxidation: alcohol -> carbonyl via NCS/DMS.
        if r.get("alcohol", 0) < 1:
            penalize(0.25, "Corey-Kim: 알코올 전구체 부족")
        if d.get("alcohol", 0) >= 0:
            penalize(0.30, "Corey-Kim: 알코올 감소 신호 부족")
    elif "friedel-crafts" in fam and "alkylation" in fam:
        # Friedel-Crafts alkylation: aromatic ring + R-X/alcohol/alkene -> arene-R.
        # Always requires an aromatic ring on reactants.
        if r.get("aromatic_ring", 0) < 1:
            penalize(0.25, "FC alkylation: 방향족 고리 전구체 부족")
    elif "finkelstein" in fam:
        # Finkelstein: R-X + NaI/KI -> R-I via SN2 on sp3 carbon. Phase 3g-1
        # upgrades this guard to use the new `alkyl_halide` SMARTS (sp3 C-X),
        # which correctly detects alkyl substrates; the prior 3e-b-3 softened
        # guard only looked at carbonyl/amine presence as a weak tiebreak.
        if r.get("alkyl_halide", 0) < 1 and r.get("aryl_halide", 0) < 1:
            penalize(0.22, "Finkelstein: halide 전구체 부족")
        if (r.get("carbonyl", 0) + r.get("amine", 0) + r.get("amide", 0)) >= 2 and r.get("alkyl_halide", 0) < 1:
            penalize(0.45, "Finkelstein: 카보닐/아민 풍부 — 단순 halide 치환이 아닐 가능성")
    # --- end Phase 3e-b-2 pattern guards ---
    # --- Phase 3e-b-3 pattern guards start (20260421_140000) ---
    # Batch 3: pick up downstream Wittig-bleed redirects that 3e-b-2 exposed.
    # After Wittig guard fired, Miyaura Boration and Corey-Fuchs became the
    # new false-positive family for ~4 cases. Both have clear reagent class
    # signatures that the test reactants lack.
    elif "miyaura boration" in fam or ("miyaura" in fam and "bor" in fam):
        # Miyaura Boration: Ar-X + B2pin2 / HBpin + [Pd] -> Ar-Bpin.
        # Requires boron source on reactant side or boron in products.
        if r.get("boron", 0) < 1 and p.get("boron", 0) < 1:
            penalize(0.22, "Miyaura Boration: boron 소스 부재")
        if r.get("aryl_halide", 0) < 1 and r.get("boron", 0) < 1:
            penalize(0.35, "Miyaura Boration: aryl halide/boron 전구체 부족")
    elif "corey-fuchs" in fam or ("corey" in fam and "fuchs" in fam):
        # Corey-Fuchs: aldehyde + CBr4/PPh3 -> dibromoalkene -> alkyne.
        # Requires phosphorus (PPh3) and typically halide source.
        if r.get("phosphorus", 0) < 1 and p.get("phosphorus", 0) < 1:
            penalize(0.25, "Corey-Fuchs: phosphorus(PPh3) 부재")
        if r.get("carbonyl", 0) < 1:
            penalize(0.35, "Corey-Fuchs: aldehyde 전구체 부족")
    # --- end Phase 3e-b-3 pattern guards ---
    # --- Phase 3g-1 pattern guards start (20260421_150000) ---
    # Leverages the new tin / zinc / alkyl_halide SMARTS. Each guard is a
    # defensive penalty on a family that is named for a specific reagent
    # class (allyl-tin for Stille/Keck, zinc enolate for Reformatsky,
    # alkyl halide for Finkelstein). Every branch is guarded by a
    # conservative threshold so that correctly-featured test cases do NOT
    # trigger a penalty.
    elif (fam.startswith("stille cross-coupling") or fam == "stille cross-coupling"
          or fam == "stille carbonylative cross-coupling"
          or (fam.startswith("stille ") and "kelly" not in fam and "carbonylative" not in fam)):
        # Stille: R-Sn(R')3 + R''-X + [Pd] -> R-R''. Requires tin on reactant
        # side (organostannane). Most Stille mispredictions lack Sn entirely.
        # Narrowed in 3g-1a to EXCLUDE "Stille-Kelly Coupling" (distinct family
        # — Stille-Kelly uses a different coupling partner and has its own tests).
        if r.get("tin", 0) < 1 and p.get("tin", 0) < 1:
            penalize(0.22, "Stille: organostannane(Sn) 부재")
        if r.get("aryl_halide", 0) < 1 and r.get("alkyl_halide", 0) < 1 and r.get("tin", 0) < 1:
            penalize(0.35, "Stille: halide/stannane 전구체 부족")
    elif "keck asymmetric allylation" in fam:
        # Keck Asymmetric Allylation: allyl-Sn + aldehyde + Ti(OiPr)4/BINOL -> homoallyl alcohol.
        # Distinct from Sakurai (silicon) and Roush (boron); tin is the discriminator.
        # Narrowed in 3g-1a to EXACT subfamily match — "Keck Radical Allylation"
        # and "Keck Macrolactonization" are distinct families with their own tests.
        # 3g-1b: softened tin penalty (0.22 -> 0.45) because some curated DB extracts
        # carry allylstannane only in reagents_text (not reactant_smiles) and were
        # being ejected from top5 coverage. Homoallyl-alcohol product signature +
        # carbonyl reactant still gives a Keck boost to compensate.
        has_homoallyl_alcohol = (p.get("alcohol", 0) >= 1 and p.get("alkene", 0) >= 1)
        if r.get("tin", 0) < 1:
            penalty = 0.45 if has_homoallyl_alcohol else 0.28
            penalize(penalty, "Keck Asymmetric Allylation: tin(allyl-Sn) 부재")
        if r.get("carbonyl", 0) < 1:
            penalize(0.35, "Keck Asymmetric Allylation: 카보닐 파트너 부족")
        if r.get("tin", 0) >= 1 and r.get("carbonyl", 0) >= 1:
            boost(1.12, "Keck Asymmetric Allylation: tin + carbonyl 조합 적합")
        if has_homoallyl_alcohol and r.get("carbonyl", 0) >= 1:
            boost(1.08, "Keck Asymmetric Allylation: homoallyl alcohol 생성물 시그니처")
    elif "reformatsky" in fam:
        # Reformatsky: alpha-halo ester + Zn -> zinc enolate -> adds to carbonyl.
        # Requires zinc OR (alkyl_halide + ester) on reactants, plus a carbonyl
        # electrophile partner. Many Reformatsky-bleed cases lack all of these.
        has_zn_or_halester = (
            r.get("zinc", 0) >= 1
            or (r.get("alkyl_halide", 0) >= 1 and r.get("ester", 0) >= 1)
        )
        if not has_zn_or_halester:
            penalize(0.25, "Reformatsky: Zn 또는 alpha-halo ester 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.35, "Reformatsky: 카보닐 electrophile 부족")
    # --- end Phase 3g-1 pattern guards ---
    # --- Phase 3g-2 pattern guards start (20260421_180000) ---
    # Heterocycle-formation family guards. Each requires its product ring
    # signature; missing product ring is a strong anti-signal. Each also
    # requires a plausible precursor (amine + 2 carbonyls for Paal-Knorr
    # Pyrrole, 1,4-dicarbonyl for Paal-Knorr Furan, aryl_halide + alkyne
    # for Larock Indole, aldehyde + beta-ketoester + amine for Hantzsch).
    elif "paal-knorr pyrrole" in fam:
        # 1,4-dicarbonyl + primary amine (often NH3) -> N-substituted pyrrole.
        # 3g-2b: dropped hard amine-reactant requirement — NH3 is often written
        # as atomic "N" which fails our amine SMARTS ([NX3;H2,H1]). Instead,
        # rely on the (1,4-dicarbonyl reactant + pyrrole product) combo as
        # the discriminator.
        if p.get("pyrrole", 0) < 1:
            penalize(0.50, "Paal-Knorr Pyrrole: pyrrole 생성물 시그니처 부족")
        if r.get("carbonyl", 0) < 2:
            penalize(0.30, "Paal-Knorr Pyrrole: 1,4-dicarbonyl 전구체 부족")
        if p.get("pyrrole", 0) >= 1 and r.get("carbonyl", 0) >= 2:
            boost(1.15, "Paal-Knorr Pyrrole: pyrrole 생성물 + 1,4-dicarbonyl 조합")
        if p.get("pyrrole", 0) >= 1 and (r.get("amine", 0) + r.get("ammonia", 0)) >= 1:
            boost(1.08, "Paal-Knorr Pyrrole: amine/NH3 시그니처 부가 신호")
    elif "paal-knorr furan" in fam:
        # 1,4-dicarbonyl + acid -> furan (no amine).
        if p.get("furan", 0) < 1:
            penalize(0.22, "Paal-Knorr Furan: furan 생성물 시그니처 부족")
        if r.get("carbonyl", 0) < 2:
            penalize(0.30, "Paal-Knorr Furan: 1,4-dicarbonyl 전구체 부족")
        # Furan synthesis should NOT consume primary amine (distinguish from Pyrrole).
        if r.get("amine", 0) >= 1:
            penalize(0.50, "Paal-Knorr Furan: 아민 존재 — Pyrrole 경로 가능성")
    elif "feist-b" in fam and "furan" in fam:
        # Feist-Benary Furan: alpha-halo ketone + beta-ketoester -> furan.
        if p.get("furan", 0) < 1:
            penalize(0.22, "Feist-Benary: furan 생성물 시그니처 부족")
        if r.get("alkyl_halide", 0) < 1:
            penalize(0.30, "Feist-Benary: alpha-halo ketone 전구체 부족")
        if r.get("carbonyl", 0) < 2:
            penalize(0.30, "Feist-Benary: 케톤/케토에스테르 전구체 부족")
    elif "hantzsch" in fam and ("dihydropyridine" in fam or "pyridine" in fam):
        # Hantzsch Dihydropyridine: aldehyde + 2 beta-ketoesters + NH3 -> DHP.
        # Product is either dihydropyridine (non-aromatic) or pyridine (oxidized).
        # 3g-2a: accept EITHER aromatic pyridine OR non-aromatic dihydropyridine
        # (added dihydropyridine SMARTS). Own DB rows were being penalized because
        # pyridine SMARTS requires aromatic [nX2;r6] and Hantzsch product N is sp3.
        # 3g-2c: amine SMARTS [NX3;H2,H1] excludes NH3 (H3); DB seeds use bare "N"
        # for ammonia. Accept amine OR ammonia as nitrogen source.
        has_dhp_product = (p.get("pyridine", 0) >= 1 or p.get("dihydropyridine", 0) >= 1)
        if not has_dhp_product:
            penalize(0.40, "Hantzsch: pyridine/DHP 생성물 시그니처 부족")
        if r.get("carbonyl", 0) < 2:
            penalize(0.30, "Hantzsch: 알데히드 + 케토에스테르 전구체 부족")
        if (r.get("amine", 0) + r.get("ammonia", 0)) < 1:
            penalize(0.30, "Hantzsch: 질소 전구체(NH3/amine) 부족")
    elif "larock" in fam and "indole" in fam:
        # Larock Indole: o-iodoaniline + internal alkyne + Pd -> 2,3-disubst indole.
        # 3g-2b: dropped hard alkyne requirement (curated DB rows put alkyne
        # in reagents_text like "diphenylacetylene"/"2-butyne"). The two-factor
        # signal becomes: aryl_halide + pyrrole product. aromatic_amine and
        # alkyne are now BOOST-only.
        if r.get("aryl_halide", 0) < 1:
            penalize(0.22, "Larock Indole: aryl halide 전구체 부족")
        if p.get("pyrrole", 0) < 1:
            penalize(0.30, "Larock Indole: indole(pyrrole) 생성물 부족")
        if r.get("aromatic_amine", 0) >= 1 and r.get("aryl_halide", 0) >= 1 and p.get("pyrrole", 0) >= 1:
            boost(1.15, "Larock Indole: o-aryl halide aniline + indole 조합 적합")
        if r.get("alkyne", 0) >= 1 and p.get("pyrrole", 0) >= 1:
            boost(1.08, "Larock Indole: alkyne + indole 생성물 파트너")
    # --- end Phase 3g-2 pattern guards ---
    # --- Phase 3g-3 isocyanide MCR cluster start (20260421_220000) ---
    # Aldol over-predicts 5 cases in 3g-2c confusion: Dakin-West, Mitsunobu,
    # Passerini, Petasis Boronic-Mannich, Ugi. Of these, Passerini (1126), Ugi
    # (1345) use [C-]#[N+] isocyanide explicitly, Petasis (1141) uses
    # phenylboronic acid [c]B(O)O. Aldol does not use either — anti-greed
    # penalty on Aldol + dedicated boost/require for Ugi/Passerini/Petasis.
    elif fam == "aldol reaction" or fam.startswith("aldol reaction"):
        # Aldol = carbonyl + carbonyl -> beta-hydroxy carbonyl. Isocyanide
        # presence is a strong anti-signal (Ugi/Passerini hallmark).
        if r.get("isocyanide", 0) >= 1:
            penalize(0.50, "Aldol: isocyanide 전구체 → Ugi/Passerini 가능성")
        # Petasis Boronic-Mannich: aryl boronic acid + amine + carbonyl.
        # Aldol doesn't use boronic acid partners — penalize when boron+amine combo.
        if r.get("boron", 0) >= 1 and r.get("amine", 0) >= 1:
            penalize(0.60, "Aldol: boron+amine 조합 → Petasis 가능성")
    elif "ugi" in fam and "multicomponent" in fam:
        # Ugi-4CR: aldehyde + amine + carboxylic acid + isocyanide -> alpha-acylamino amide.
        # NH3 uses "N" which is invisible to amine SMARTS [NX3;H2,H1] (H3), so accept amine|ammonia.
        if r.get("isocyanide", 0) < 1:
            penalize(0.30, "Ugi MCR: isocyanide 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.30, "Ugi MCR: 알데히드/케톤 전구체 부족")
        if (r.get("amine", 0) + r.get("ammonia", 0)) < 1:
            penalize(0.30, "Ugi MCR: 아민/NH3 전구체 부족")
        if r.get("carboxylic_acid", 0) < 1:
            penalize(0.30, "Ugi MCR: 카복실산 전구체 부족")
        # Positive combo boost when all 4 partners are in reactants.
        if (r.get("isocyanide", 0) >= 1 and r.get("carbonyl", 0) >= 1
                and (r.get("amine", 0) + r.get("ammonia", 0)) >= 1 and r.get("carboxylic_acid", 0) >= 1):
            boost(1.25, "Ugi MCR: isocyanide + aldehyde + amine/NH3 + 산 4-성분 조합")
    elif "passerini" in fam:
        # Passerini-3CR: aldehyde + carboxylic acid + isocyanide -> alpha-acyloxy amide.
        if r.get("isocyanide", 0) < 1:
            penalize(0.30, "Passerini MCR: isocyanide 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.30, "Passerini MCR: 알데히드 전구체 부족")
        if r.get("carboxylic_acid", 0) < 1:
            penalize(0.30, "Passerini MCR: 카복실산 전구체 부족")
        if (r.get("isocyanide", 0) >= 1 and r.get("carbonyl", 0) >= 1
                and r.get("carboxylic_acid", 0) >= 1):
            boost(1.22, "Passerini MCR: isocyanide + aldehyde + 산 3-성분 조합")
    elif "petasis" in fam and ("mannich" in fam or "boronic" in fam):
        # Petasis Boronic Acid-Mannich ONLY (not Petasis-Ferrier Rearrangement or
        # Petasis-Tebbe Olefination which share the "petasis" substring but have
        # totally different mechanisms). Requires aryl/vinyl boronic acid + amine + carbonyl.
        if r.get("boron", 0) < 1:
            penalize(0.30, "Petasis-Mannich: boronic acid 전구체 부족")
        if (r.get("amine", 0) + r.get("ammonia", 0)) < 1:
            penalize(0.30, "Petasis-Mannich: 아민/NH3 전구체 부족")
        if r.get("carbonyl", 0) < 1:
            penalize(0.30, "Petasis-Mannich: 카보닐 전구체 부족")
        if (r.get("boron", 0) >= 1 and (r.get("amine", 0) + r.get("ammonia", 0)) >= 1
                and r.get("carbonyl", 0) >= 1):
            boost(1.20, "Petasis-Mannich: boron + amine/NH3 + carbonyl 3-성분 조합")
    # --- end Phase 3g-3 isocyanide MCR cluster ---
    # --- Phase 3g-4 Benzoin/Stetter anti-greed start (20260421_233000) ---
    # 3g-3 analysis: Aldol penalty successfully demoted Aldol on Ugi/Passerini/Petasis SMILES,
    # but Benzoin-and-Retro-Benzoin-Condensation took top1 on Petasis_1141/1146, Ugi_1345/1346
    # and Stetter Reaction took top1 on Passerini_1126/1127. Both are carbonyl-heavy. Apply
    # the same anti-greed signal (isocyanide → Ugi/Passerini; boron+amine → Petasis).
    # Own-family audit: all Benzoin (6) and Stetter (4) DB rows have iso=0/bor=0/amn=0, so
    # these penalties cannot hurt their own seeds.
    elif "benzoin" in fam and "condensation" in fam:
        if r.get("isocyanide", 0) >= 1:
            penalize(0.45, "Benzoin Condensation: isocyanide → Ugi/Passerini 가능성")
        if r.get("boron", 0) >= 1 and (r.get("amine", 0) + r.get("ammonia", 0)) >= 1:
            penalize(0.55, "Benzoin Condensation: boron+amine → Petasis 가능성")
    elif fam == "stetter reaction" or fam.startswith("stetter reaction"):
        if r.get("isocyanide", 0) >= 1:
            penalize(0.45, "Stetter Reaction: isocyanide → Ugi/Passerini 가능성")
        if r.get("boron", 0) >= 1 and (r.get("amine", 0) + r.get("ammonia", 0)) >= 1:
            penalize(0.55, "Stetter Reaction: boron+amine → Petasis 가능성")
    # --- end Phase 3g-4 Benzoin/Stetter anti-greed ---
    # --- Phase 3g-5 Grignard/Regitz anti-greed start (20260422_000000) ---
    # 3g-4 analysis: Benzoin/Stetter demoted successfully, but residual greed shifted again:
    # Grignard Reaction captured Passerini_1126 top1, Regitz Diazo Transfer captured
    # Passerini_1127 top1. Both have zero iso-carrying DB rows (probe_3g_5_rows.py:
    # Grignard 0/3, Regitz 0/4). Penalize isocyanide presence only.
    elif fam == "grignard reaction" or fam.startswith("grignard reaction"):
        if r.get("isocyanide", 0) >= 1:
            penalize(0.45, "Grignard Reaction: isocyanide → Ugi/Passerini 가능성")
    elif fam == "regitz diazo transfer" or fam.startswith("regitz diazo transfer"):
        if r.get("isocyanide", 0) >= 1:
            penalize(0.45, "Regitz Diazo Transfer: isocyanide → Ugi/Passerini 가능성")
    # --- end Phase 3g-5 Grignard/Regitz anti-greed ---
